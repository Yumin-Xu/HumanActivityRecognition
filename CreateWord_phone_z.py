...
def if_in_count_phone_accel(x, c):
    data1 = OpenData_phone_accel.csv_new_file(x)
...
def if_in_count_phone_gyro(x, c):
    data2 = OpenData_phone_gyro.csv_new_file(x)
...
def create_word_phone_accel_z(x, c):
    data1 = if_in_count_phone_accel(x, c)

    x1_data1 = pd.array(data1['z'].loc[data1.index[1]: data1.index[len(data1) - 1]])
    x2_data1 = pd.array(data1['z'].loc[data1.index[0]: data1.index[len(data1) - 2]])
...
def create_word_phone_gyro_z(x, c):
    data2 = if_in_count_phone_gyro(x, c)

    x1_data2 = pd.array(data2['z'].loc[data2.index[1]: data2.index[len(data2) - 1]])
    x2_data2 = pd.array(data2['z'].loc[data2.index[0]: data2.index[len(data2) - 2]])
