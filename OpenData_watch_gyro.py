...
def path():
    var1 = 0
    file_path = pd.Series(len(ReadRawData.read_data_watch_gyro()))
    for file in range(0, len(ReadRawData.read_data_watch_gyro())):
        file_path[var1] = ReadRawData.read_data_watch_gyro()[file]
        var1 += 1
    return file_path
...
def csv_file(x):
    p = ReadRawData.read_csv_watch_gyro()[x]
    f1 = pd.read_csv(p)
